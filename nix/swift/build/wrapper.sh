...
if [[ -n "@coreutils_bin@" && -n "@gnugrep_bin@" && -n "@gnused_bin@" ]]; then
    PATH="@coreutils_bin@/bin:@gnugrep_bin@/bin:@gnused_bin@/bin"
fi
...
# Strip -Werror=unguarded-availability flags added by Apple SDK which break swiftc
NIX_CFLAGS_COMPILE_@suffixSalt@="${NIX_CFLAGS_COMPILE_@suffixSalt@//-Werror=unguarded-availability-new/}"
NIX_CFLAGS_COMPILE_@suffixSalt@="${NIX_CFLAGS_COMPILE_@suffixSalt@//-Werror=unguarded-availability/}"
NIX_LDFLAGS_@suffixSalt@="${NIX_LDFLAGS_@suffixSalt@//-Werror=unguarded-availability-new/}"
NIX_LDFLAGS_@suffixSalt@="${NIX_LDFLAGS_@suffixSalt@//-Werror=unguarded-availability/}"

# Fix SDK 26 injecting clang-21 headers (which Swift 6.2 clang 17 doesn't understand)
# We use sed to reliably remove the pair `-isystem /nix/store/.../resource-root/include`
export NIX_CFLAGS_COMPILE_@suffixSalt@=$(echo "$NIX_CFLAGS_COMPILE_@suffixSalt@" | sed -E 's|-isystem /nix/store/[^ ]*-clang-wrapper-[^ ]*/resource-root/include||g' | sed -E 's|-resource-dir=[^ ]*||g')
export NIX_CFLAGS_COMPILE_BEFORE_@suffixSalt@=$(echo "$NIX_CFLAGS_COMPILE_BEFORE_@suffixSalt@" | sed -E 's|-isystem /nix/store/[^ ]*-clang-wrapper-[^ ]*/resource-root/include||g' | sed -E 's|-resource-dir=[^ ]*||g')
    ...
    for ((i = 1; i <= $#; i++)); do
        ...
        case "$val" in
            ...
            -Werror=unguarded-availability*)
                # Apple SDK injects these, which swiftc doesn't recognize
                ;;
...
finalParams=()
for p in ${extraBefore[@]+"${extraBefore[@]}"} ${params[@]+"${params[@]}"} ${extraAfter[@]+"${extraAfter[@]}"}; do
    if [[ "$p" == "-Werror=unguarded-availability"* ]]; then
        continue
    fi
    finalParams+=("$p")
done
...
if (( "${NIX_CC_USE_RESPONSE_FILE:-@use_response_file_by_default@}" >= 1 )); then
    ...
    if [ ${#finalParams[@]} -gt 0 ]; then
        printf "%q\n" "${finalParams[@]}" > "$responseFile"
    else
        touch "$responseFile"
    fi
    unset NIX_CC NIX_CFLAGS_COMPILE NIX_LDFLAGS NIX_CFLAGS_COMPILE_BEFORE NIX_LDFLAGS_BEFORE C_INCLUDE_PATH CPLUS_INCLUDE_PATH CPATH
    (exec -a "$progName" "$prog" "@$responseFile")
else
    unset NIX_CC NIX_CFLAGS_COMPILE NIX_LDFLAGS NIX_CFLAGS_COMPILE_BEFORE NIX_LDFLAGS_BEFORE C_INCLUDE_PATH CPLUS_INCLUDE_PATH CPATH
    exec -a "$progName" "$prog" ${finalParams[@]+"${finalParams[@]}"}
fi
