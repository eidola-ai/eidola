...
| **Grafana Cloud** | Telemetry (OpenTelemetry traces, metrics, logs) | Operational telemetry only, limited to generic, unidentifiable fields. Grafana receives counters and histograms aggregated across requests (latency by route and operation, token totals, stream and webhook outcomes), never a per-request record — **no traces are sent for ordinary traffic, on any surface.** The exception is a request that explicitly asks to be traced by sending a `traceparent`, which is how we profile our own traffic in production. Never content, never an account. |
