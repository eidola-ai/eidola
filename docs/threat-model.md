...
**What also stops them:** *Channel binding.* The per-handshake nonce gives freshness but binds only the long-term TLS *key* (cert SPKI), which would leave a gap against an attacker who has **exfiltrated** that key: they could terminate the client's connection with the stolen key — reading the plaintext, since an active MITM holding the signing key derives the session keys regardless of TLS 1.3 forward secrecy — while relaying a fresh, nonce-bound report from the enclave's public endpoint. To close that, the enclave folds the RFC 9266 `tls-exporter` of the session it terminates into `REPORT_DATA`, and the client checks it against *this* session's exporter. A relaying MITM terminates a different TLS session than the one it relays to the enclave, so the two exporters differ and the check fails — even with the stolen key, because producing a report bound to *our* session's exporter requires the genuine hardware terminating *our* session.

**Residual exposure:** Channel binding fully removes the key-exfiltration avenue, leaving only defeat of the confidential-compute hardware itself (see [N5](#n5-microarchitectural-side-channels-against-confidential-compute-hardware)). The one caveat is rollout: the client enforces the binding whenever the enclave provides it, but only *requires* it once the upstream enclave reliably emits it (`require_channel_binding`). During the transition, an enclave build that does not yet bind the channel falls back to the nonce-only guarantee, whose residual rests on the TLS key staying sealed inside the enclave.
...
**Residual exposure:** Same as A3: the RFC 9266 `tls-exporter` channel binding ties the attestation to the live TLS session, so an attacker who has exfiltrated the TLS private key can no longer relay a fresh report — their client-facing session has a different exporter than the one they relay to the enclave. The residual collapses to defeating the confidential-compute hardware, subject to the same rollout caveat (the binding is enforced when present and required once the upstream enclave reliably emits it).
