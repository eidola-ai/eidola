...
**What stops them:** The boundary between the *linked* account surface and the *unlinked* inference surface is enforced at the type level in the server, and the inference surface uses anonymous credentials (Privacy Pass ACT). The operator can see *that* a credential was redeemed, never *whose* credential it was. Inference content never reaches the operator's logging surface at all. Telemetry is restricted to generic, unidentifiable fields and carries no per-request records on any surface: the operator sees counters and histograms aggregated across requests, never an individual one. The operator cannot single out a request for closer inspection either — a per-request trace is produced only when the *requester* asks for one by sending a `traceparent`, which is a client-side decision the operator has no way to make on someone else's behalf.
...
**What stops them:** Binding between the TLS key and a *fresh* enclave attestation. The client issues an inline `GET /.well-known/tinfoil-attestation?nonce=<hex>` (a fresh random nonce per handshake) over the same TCP+TLS connection as the real request. It hashes the exact endorsed crypto-material and device-evidence section bytes and verifies `REPORT_DATA = sha256(report-data-v1 URI ‖ nonce ‖ crypto_material_hash ‖ device_evidence_hash)`, with the crypto section's `tls` item equal to `sha256(SPKI(peer_cert))`. An attacker presenting a valid certificate for the same hostname without the matching attestation fails the check; an attacker presenting a stale or replayed attestation document fails either the SPKI binding or nonce check.
...
- **Source-bootstrapped reproducible builds for the Linux OCI images** (StageX). This eliminates an entire class of "what was actually in the toolchain" risks: every binary in the chain — down to the assembly that built the c compiler — is built from source whose hash is committed and verified.
- **Hermetic builds for the desktop artifacts** (Nix — the macOS universal binaries and the Linux GUI). These are *hermetic*, not fully source-bootstrapped in the StageX sense, but provide similar properties for components of the build environment above the OS. On macOS, the operating system and parts of its toolchain are additionally opaque inputs we must accept, because cross-compiling to macOS from Linux is not viable today. (The Linux GUI is Nix-built rather than StageX because a desktop GUI must be a glibc dynamic binary to interoperate with the host GPU stack, which StageX's musl-only toolchain cannot produce.) See [gaps.md](gaps.md#build-chain-opacity).
