...
## Inline composer — input as continuation, not chrome

The chat has no separate input bar. The composer (`Input::new(&self.prompt_state).appearance(false)`) is the last child of `messages_col` inside the scroll container — it shares the prose column, prose typography (17px / 1.65× via `Styled` overrides on the Input itself, since gpui-component's `Input` hardcodes a 1.25-rem line-height that we have to refine over), and the same `theme.background` as the rest of the page. The result reads as a continuation of the journal rather than a chat-app chrome.
...
**Indefinite growth, single scroll surface.** `InputState::auto_grow(1, 10_000)` is how we disable the input's *internal* scrollbar without giving up `auto_grow`'s row-tracking: the cap is large enough that no realistic prompt hits it, so the input keeps growing with content and the *outer* `div().id("scroll").overflow_y_scroll()` handles overflow as one continuous unit (input + all preceding messages). A plain `multi_line(true)` `PlainText` mode would have a fixed `rows: 1` and overflow-scroll inside the input; `auto_grow(1, 10_000)` is the way to keep one scroll surface.

The min height defaults to one row — when there's nothing typed, the composer is a one-line area, not a sheet that fills the window. That's intentional for now; making clicks anywhere on the page focus the editor would need either `min_h(relative(1.))` on `messages_col` plus `flex_grow()` on the wrapper, or a `flex_grow` wrapper with an explicit `on_mouse_down` focus handler. We tried both — it worked — but the user opted to keep the simpler one-row composer until there's a clear need.

**Half-viewport bottom padding.** The composer's `Input` is given `pb(window.viewport_size().height * 0.5)` (computed at render time, applied via `Styled` after `.p_0()` clears the size-derived defaults). The cursor never sits pinned to the bottom edge — there's always a half-page of empty space below the active line, so the parent scroll can keep the typing zone in the comfortable middle of the viewport as content grows. `messages_col` no longer carries `pb_8`; this internal padding on the composer is the bottom breath. Computed in `Render` rather than as a constant because it tracks live window resizes.

**Cmd+Return dispatch.** ⌘↩ has *two* paths to `submit`, depending on whether the composer has focus:

1. **Focused composer (the common case).** `gpui-component`'s `InputState` registers `secondary-enter → Enter { secondary: true }` in the `Input` key context. That binding wins over our `cmd-enter → Send` (ChatView) binding because the `Input` context is innermost in the focus chain. The Input handler always inserts a newline in multi-line mode and emits `InputEvent::PressEnter { secondary: true }`. `ChatView::new` subscribes to that event via `cx.subscribe_in(&prompt_state, window, ...)` and calls `submit(&Send, ...)` when `secondary == true`. The newline that just got inserted gets trimmed by `submit`'s `value.trim()` before the prompt goes upstream.
2. **No composer focus (fallback).** If focus has moved off the composer (e.g. the user clicked into a markdown body to select text), the Input binding doesn't apply and `cmd-enter → Send` (ChatView context) fires the `Self::submit` listener registered on the v_flex root. Behavior tests use this path directly via `focus_handle.dispatch_action(&Send, ...)`.

Both paths land in the same `ChatView::submit`, which is idempotent under streaming (`if self.streaming.is_some() { return; }`). The composer remains visible and editable during streaming, but submit is silently a no-op until the stream completes — known UX hole, fine for first iteration.

**Focus on construction.** `ChatView::new` calls `prompt_state.update(cx, |s, cx| s.focus(window, cx))` so the cursor lands in the composer when a window opens. The view's own `focus_handle` is still `track_focus`ed by the root v_flex (it's what behavior tests dispatch through), but no longer focused at construction — the composer takes that role.

**Test wrapping invariant.** Behavior tests must wrap the view in `gpui_component::Root` (see `tests/behavior.rs::open_view`). Production already does this in `lib.rs`; tests previously got away without it because the prior chat layout never gave the input focus, so `InputState::on_blur` (which calls `Root::update` and panics on a non-`Root` window root) never fired. Now that the composer is focused on construction, every chat-bearing test window must be `Root`-wrapped or it'll panic at window close.
