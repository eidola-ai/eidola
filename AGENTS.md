...
The per-handshake nonce guarantees *freshness* — the enclave folds our random nonce into a freshly collected `REPORT_DATA`, so a stale or captured document can't be replayed against a different nonce, and we know a live, genuine CC machine currently holds the cert key. The nonce alone binds the long-term TLS *key* (cert SPKI), not the live TLS *session*, which would leave a key-exfiltration gap: an attacker holding the stolen key could actively MITM (deriving the session keys despite TLS 1.3 forward secrecy) and relay a fresh nonce-bound report from the enclave's public endpoint. The **channel binding** closes it — the enclave folds the RFC 9266 `tls-exporter` of the session it terminates into `REPORT_DATA`, and the verifier checks it against this session's exporter; a relaying MITM terminates a different TLS session than it relays to the enclave, so the two exporters differ and the check fails (even with a stolen key, since producing a report bound to *our* session's exporter needs the genuine hardware terminating *our* session). This depends on the **forked reqwest** at `../reqwest` (`[patch.crates-io]` in the workspace `Cargo.toml`) which exposes the session exporter via `TlsInfo::tls_exporter()` — upstream reqwest only surfaces the peer cert. Channel binding is enforced opportunistically while current Tinfoil enclaves still predate it (`require_channel_binding` defaults off; flip on once the upstream enclave reliably emits the binding). Separately, the fresh document is not yet *fully* self-contained — it omits the VCEK — so the ATC fallback remains in place; once Tinfoil folds the VCEK into the document, that path will go cold and can be removed entirely.
...
- `artifact-manifest.json` (`schema_version: 1` — integer; see `docs/trust-root.md` for why our schema versions are integers, not semver) records expected OCI digests, the macOS universal CLI binary and GUI `.app` bundle Nix `narHash`es, and enclave measurements (SEV-SNP + TDX + cmdline) with type/platform metadata; the enclave block shape matches the Tinfoil `snp-tdx-multiplatform/v1` predicate so `tinfoil-build.yml` can project it directly into `tinfoil-deployment.json`. the Build & Verify Artifacts workflow verifies the full file by merging digests captured from the real OCI and macOS build jobs and recomputing enclave measurements from `tinfoil-config.yml`
