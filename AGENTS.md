...
**Branch flow (`next` → `main`):** `next` is the integration trunk; `main` is the verified, promotion-only branch. Feature/dependency PRs target `next` (Dependabot is pointed at `next`). The cheap workflows (`rust-checks`, `cla`, `dependency-review`, `codeql`, `zizmor`, `pinact`) run on `next` PRs. The expensive `Build & Verify Artifacts` (`artifacts.yml`) chain does **not** run on `next` PRs — it is scoped to `main` (push + the `next` → `main` promotion PR) and is the required gate for `main`. `enforce-branching-strategy.yml`'s `base-guard` job is the enforcement: on any PR into `main` it passes (green) **only** for this repo's `next` → `main` promotion PR, and for every other PR it retargets onto `next` and stays red — deliberately, even when the retarget succeeds. The job's success conclusion is recorded against the PR's *head SHA*, not its base, so a green on a non-promotion head would persist after the base moves to `next`; an attacker could then flip the base back to `main` and merge in the window before a corrective run retargets again, reusing that green plus the expensive `main` checks that already ran on the same SHA. Keeping non-promotion heads red closes that reuse: as a required check on `main` it blocks any non-`next` source from merging. The lingering red is cosmetic on `next` (the check is not required there) but load-bearing on `main`. Promotion: open a `next` → `main` PR (this is the collaborative surface where the full `Build & Verify Artifacts` chain runs and integration/nondeterminism debugging happens); when it's approved and green, a maintainer comments `/promote` and `promote.yml` fast-forwards `main` to the validated `next` head SHA. The push is non-force, so GitHub rejects anything that isn't a fast-forward — `main` stays a strict ancestor of `next` (`main ⊆ next`), SHA-identical to a real `next` commit, and lands exactly what the Build & Verify Artifacts chain validated. `branch-guard.yml` continuously asserts `main ⊆ next` (on push to either branch + every 6h) as a backstop against out-of-band divergence. **GitHub config (not in-repo):** on `main`, require the `Build & Verify Artifacts` checks, restrict direct pushes to the promotion GitHub App as the sole ruleset bypass actor (its App ID + private key live only on the `main`-restricted `promote` Environment, so only `promote.yml` running from `main` can mint a token — see `docs/contributing/promotion-security.md`), and block force-pushes; on `next`, block force-pushes and deletions; `Build & Verify Artifacts` is required for `main` but **not** for `next`. The single human rule: never commit to `main` directly — everything funnels through `next`.
