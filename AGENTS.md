...
The enclave's own `/.well-known/tinfoil-attestation?v=3` document is the source of truth. ATC is the single fallback target — the verifier never talks to AMD KDS, and v2 attestation documents are no longer supported. `trusted_ark_der` / `trusted_ask_der` only feed the SEV-SNP attestation chain verifier; they are **not** added to any TLS root store. Test deployments that need TLS to trust a custom CA (e.g. the tinfoil shim mock) install that CA in the developer's system trust store. Pure Rust dependencies (`sev`, `x509-cert`, `der`, `tower`, `hyper`, `hyper-util`, `httparse`) — no OpenSSL.
