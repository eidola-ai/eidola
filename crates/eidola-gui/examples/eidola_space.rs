...
impl SpaceView {
    ...
    fn render_minimap(
        ...
        // what's currently selected. The empty title-bar reserve above the root
        // is part of the scrollable content, so include it in the denominator
        // (and as a spacer below) — that keeps the dark visible-window a
        // consistent size as it slides off the top, instead of "growing" through
        // the reserve.
        let reserve = TITLE_BAR_RESERVE.as_f32();
        let longest = reserve + max_path_height(&self.root, &bounds, slot_h);
