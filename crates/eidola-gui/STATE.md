...
| `AccountStore` | balances, prices, subscription standing, account lifecycle, checkout | The checkout *poll* is view-owned (see scoping); its result lands here via invalidation. The subscription cell is a live read that persists nothing, so no `Change` ever invalidates it — and the balance moves on a webhook nothing local commits for either, so **both are re-read when the Account pane is selected** (`SettingsView::sync_active_pane` → `AccountView::pane_activated`; the panes are all built at window creation, so construction is not activation) and by the section's own always-offered re-check. **An identity switch clears it**: `request_verify_account` / `request_account_create` call `forget_account_scoped_state` as the switch begins, so one account's billing standing can never stand under another's name — clearing rather than refreshing, because showing nothing while a surface re-reads is honest where showing the previous account's answer is not |
