...
- **Only the mutations wait on that insert; the frame never does.** The insert and the first save are independent work on one runtime, so nothing orders them — and losing that race is not a delay but a loss: the save is refused for a space that then exists, and its failure completion's reload takes the thought the reader had already typed off the screen with it. So the entity holds the creation's state (`Space::creation`) and every save-side mutation awaits it **inside its own task**, after the draft has already been accepted on the frame. That is the shape the general rule takes here: *a write into state another operation is still creating orders itself behind that operation, at the layer that owns the write* — never by blocking the surface, and never by hoping the round trip is slower than the reader. A refusal is handed to the waiters as the refusal, so a save against a space that was never created fails for the reason it actually failed.
