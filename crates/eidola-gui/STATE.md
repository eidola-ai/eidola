...
**A view must render `Failed`, not just `Loading`/value — "Failed is not empty."** A failed *initial* load (`Failed { prior: None }`) whose `list()` returns an empty slice must **not** read as a plausible-empty surface with live controls (an empty roster with a live Add, an empty registry that reads as "Default missing") — that hides a real error behind a normal-looking UI, and any load-once cell (`ensure`-style, which declines to re-fetch once a `Failed` exists) then stays blank forever with no way back. Render the error **plus an explicit Retry** that re-fetches (the shared `participants::load_error_panel` helper is the house pattern; the inspector's Participants section, the Space Templates pane, the Library and the space window's own transcript all use it). **The rule bites hardest where the failed cell is also what mints the controls**: a space whose initial transcript read fails has no posts *and* no composer, so the page is not merely wrong, it is inert — nothing on it can cause another read. A retry is the only affordance, which is why the surface owes one. A failed *refresh* over existing data (`Failed { prior: Some }`) keeps the stale value shown and adds a quiet retry — never a blank. **The quiet retry is the half most easily skipped**, because the surface looks fine without it: the value is on screen, so the omission costs nothing visible and silently makes the data undated. It is owed even where the surface has other live controls — a *write* is not a way to re-run a *read*, and a reader with nothing to write has no way to ask again at all (the space window's transcript is the worked example: `library.rs`'s "Couldn't refresh — retry" strip is the house wording). Per-operation state (a write's `op_error`) must be **keyed the same way its snapshots are** (per-space for a space-keyed store), or two windows cross-contaminate each other's banners.
...
| `AccountStore` | balances, prices, subscription standing, the required-terms snapshot, account lifecycle, checkout | The checkout *poll* is view-owned (see scoping); its result lands here via invalidation. The subscription cell is a live read that persists nothing, so no `Change` ever invalidates it — and the balance moves on a webhook nothing local commits for either, so **both are re-read when the Account pane is selected** (`SettingsView::sync_active_pane` → `AccountView::pane_activated`; the panes are all built at window creation, so construction is not activation) and by the section's own always-offered re-check. **An identity switch clears it**, via `account_identity_changed` (clear + re-read) called on the **commit**, never on the attempt: creating and linking both refuse outright when credentials already exist, so clearing when the request went out blanked the cell for an operation that never happened. Clear *and* refresh together — a bare refresh leaves the previous account's value visible as `Loaded { stale }`, which on a billing surface is the wrong account's answer under the right account's name. The **terms** cell is the same shape for the same reason — public data that nothing on the bus invalidates — so the consent surface asks for it when it opens (`refresh_terms`) and hands the snapshot it rendered straight back to `request_account_create`, which is what keeps agreement and submission describing the same bytes. **A consent surface's own "I agree" state is view-owned and holds the snapshot's `(document, sha256)` pairs, never a `bool`** — the store cell is refreshed on the view's own schedule, so a flag would go on meaning "checked" across a snapshot it was never checked for; see `crates/eidola-gui/AGENTS.md` → Onboarding |
...
- **An optimistic edit is a promise the *failure* exit has to keep.** The same shape without the bounding re-read is a fake state with a longer fuse: `SpacesStore`'s rename/archive applied their edit, then dropped the write's `Err` — so a refused rename left the Library, the window title, and the inspector's title field all reading a name nothing persisted, until an unrelated refresh took it back with no explanation (silence read as success again, one indirection out). Optimism is only honest when the operation reconciles the cell on **every** exit and says why: the write's refusal in `op_error`, the truth from the re-list. Where the write's answer is *not* an `Err` — app-core's `archive_space` answers `Ok(false)` when it changed nothing — the operation still owes the **report**, because the row it removed was not its to remove; what it does not owe is the **undo**, because `Ok(false)` says the space was already archived or already gone, so the removal was *right* and merely not this operation's doing. Reading that as a refusal let a child's archive reinsert a row its parent's successful subtree archive had already closed. So a write answers in three states and not two (`WriteVerdict`: applied, already-so, refused), and only the last takes an edit back. And the promise outlives the mechanism that usually keeps it: when the re-list *also* fails, `Failed { prior }` keeps the pre-resolve rows on screen (correctly — "Failed is not empty"), which for an optimistic store means the edit the database just refused. Reconciliation-by-re-read is exactly what is missing in that quadrant, so capture the snapshot the edit replaced and restore it on the refused exit — before resolving, so a successful re-read still supersedes it, and only for a *refused* write, since one that landed durably is honest to keep showing.
- **Independent operations need keyed slots, and their reports need the same key.** A supersede slot shared by operations that are not alternatives is the loss above wearing a scheduler's clothes: the second replaces the first, and an in-flight write's continuation (its `op_error`, its re-list) dies with it while the core write runs on — or, if the first was never polled, the operation simply never happens. Key by whatever makes two of them independent (`SpacesStore`: the space id; `Space`: the turn seq), and defer a pending refresh to the **last** key settling, not the first. The **op-error slot takes the same key**: a store-wide slot read by a per-space surface cannot distinguish "no refusal" from "someone else's refusal replaced mine", which is the honest-states rule broken by an unrelated space's failure. What remains keyed *together* is genuinely one control's writes (two renames of one space), where last-wins is the intended reading and the residual is one lost message about the space the winner also reports on (PR #275's codex finding). **And a key several *doors* open onto is not last-wins at all**: where the second call is the same request with the same argument — Settings ▸ Backends ▸ Local's Retry, the curated catalog's Download and the paste-a-URL row all start one download of one URL — there is no newer value for a successor to carry, so supersede can only drop a working operation's continuation (or, unpolled, lose it outright). Make re-entry on a pending key a **no-op** in the store (`UpdateStore::check_now`'s flag and `Space`'s exclusive `post_runner` are the same shape), and let the surfaces gate their controls as *presentation* over an operation that is already safe — a control gated door-by-door leaves every door added later to remember the rule.
- **Concurrency turns an optimistic cell into a shared one — and the two rules above have to compose.** Keyed slots let several writes edit one snapshot at once, so: (1) **the resolving read must be *taken* after the last write, not merely applied then** — deferring *which captured listing gets used* is not enough, because every captured listing was read right after its own operation's write, which may be long before a sibling's commits; the operation that happens to be last then resolves the cell with a snapshot predating an accepted write (and a failed read preserves it as `prior`, hiding a durable write behind a refresh error). Issue the read only once no write is in flight and the property is structural rather than a race one loses. The deferral re-arms recursively — a write starting during that read drops it and owes the next — which is bounded and honest: the newest batch-end read wins, and until it lands the cell shows the writes' own edits. (2) **A refused write takes back its own edit as an inverse, never as a snapshot of the whole value — and the inverse stores no position.** Siblings compose by delta; an absolute snapshot erases a concurrent edit that has not settled, and a snapshot taken *after* another op's edit resurrects that op's refused edit when it is restored later. **And where an inverse cannot land it is owed rather than lost**: a sibling can hold the very row an inverse has to correct — an ancestor's archive removes a delegated child while that child's own rename is settling, and the two are different mutation keys, so neither supersedes the other and the row comes back only if that archive is itself refused. So an inverse answers whether it landed, and one that found nothing is retried at the end of the batch: the first moment every refused removal has put its rows back, and the last before the resolving read. That is why inverses are `Fn` and idempotent — they address rows by id and *set* values rather than applying deltas, so a retry that finds its work already done changes nothing. **The forward edit is owed the same way**, because a sibling can hide a row before an edit as easily as before an inverse: a rename started from a child's own inspector while an ancestor's archive holds that child records neither a value nor an inverse, and if it is *accepted* the archive's refused restore puts the row back under the name the database has replaced. Nothing can re-derive it there — the re-list is the failure, and the row is the only place a title lives — so the operation that earned it carries it (`Optimism::owed`), applied at the batch's end after the inverses and only where the write was accepted. The archive edit owes the same way in the mirror case, so a sibling's refused restore cannot reopen a conversation this operation closed. A *positional* inverse fails the same way one level down: a recorded slot is stale once a sibling's removal shifts the collection, so two undone removals can seat their rows in an order the database never held. Where the collection has a sort key its rows carry (the Library index: `last_activity_at DESC`), derive the seat from the row; where identity suffices (a title), key by id. Position that is never stored cannot be stale. (3) A write that supersedes another on the same key **undoes the loser's edit before applying its own** — the loser's inverse dies with its cancelled task, and the winner would otherwise restore the loser's optimistic value. The inverse is not a re-application of optimism onto fetched truth: it runs only while the cell still stands on the pre-fetch value, and a fetched value replaces the cell wholesale with no inverse outstanding.
...
- **A space is created when its window opens, not when its first message sends.** ⌘N mints the id client-side (`SpacesStore::create`) and hands back a registered entity in the same breath; the row — the default template instantiated — commits behind the window, whose page is answered-and-empty from the first frame, so the blank page stays instant and every per-space surface (participants, settings) works from birth. The insert is owned by the store, not the window, so closing the window a keystroke later still leaves a whole space; a refused insert drops the registry entry and the entity says so (`Space::row_is_gone`) rather than showing a composer that would write nowhere.
- **And it goes with its window when nothing was ever done with it.** The store owns the disposal for the same reason it owns the insert — the window that triggered it is already gone — and the layering is the general one: the GUI answers only *was that the last window* (the entity's own window list), while every judgement about whether the space is untouched belongs to app-core, inside the transaction that deletes. A close arriving over an in-flight insert is **deferred to the insert's completion**, never skipped: a write into state another operation is still creating orders itself behind that operation, and so does a delete.
- **A terminal negative is not a cell to refresh.** Once a surface knows the thing it describes does not exist, the honest state is settled and a reload can only destroy it: `Loadable::to_loading` turns a valueless `Failed` into `Loading`, and a read that is going to refuse resolves nothing — leaving a spinner where an error was. So a read against a settled-negative subject does not start. The general rule: *a debt-driven reload owes its restart only where a later read could learn something.*
- **A destructive operation never races a write the client has already issued.** `bridge` cancels nothing, so a core call outlives the gpui task that issued it — which means "the task is gone" is not "the write is done", and a delete decided from the first reading destroys the second. The rule that falls out: *ask whether anything is still writing at the last moment every owner is alive to answer, and let a destructive act proceed only on silence.* Here that moment is the view's release and the owners are four (the `Space` entity, and the three stores holding per-space write slots); erring towards "still writing" only keeps state, which is the direction a delete must always be wrong in.
- **Only the mutations wait on that insert; the frame never does.** The insert and the first save are independent work on one runtime, so nothing orders them — and losing that race is not a delay but a loss: the save is refused for a space that then exists, and its failure completion's reload takes the thought the reader had already typed off the screen with it. So the entity holds whether its id is known to name a row (`Space::row`, a `RowState`) and every save-side mutation — and the transcript load — awaits it **inside its own task**, after the draft has already been accepted on the frame. That is the shape the general rule takes here: *a write into state another operation is still creating orders itself behind that operation, at the layer that owns the write* — never by blocking the surface, and never by hoping the round trip is slower than the reader. A refusal is handed to the waiters as the refusal, so a save against a space that was never created fails for the reason it actually failed.
...
| Balances, prices, subscription standing, required terms, account lifecycle | global | `AccountStore` |
