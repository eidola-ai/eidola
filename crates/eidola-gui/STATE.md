...
| `AccountStore` | balances, prices, subscription standing, account lifecycle, checkout | The checkout *poll* is view-owned (see scoping); its result lands here via invalidation. The subscription cell is a live read that persists nothing, so no `Change` ever invalidates it — the surface showing it refreshes on open |
