...
- **Concurrency turns an optimistic cell into a shared one — and the two rules above have to compose.** Keyed slots let several writes edit one snapshot at once, so: (1) **only the last operation in flight may resolve the cell**, because an earlier sibling's fetched listing was read *before* a still-running write committed — landing it drops a change that did land, and a later failed re-read then preserves that stale listing as `prior`, hiding a durable write behind a mere refresh error. Nothing is lost by deferring: a later sibling's read is strictly fresher. (2) **A refused write takes back its own edit as an inverse, never as a snapshot of the whole value.** Siblings compose by delta; an absolute snapshot erases a concurrent edit that has not settled, and a snapshot taken *after* another op's edit resurrects that op's refused edit when it is restored later. (3) A write that supersedes another on the same key **undoes the loser's edit before applying its own** — the loser's inverse dies with its cancelled task, and the winner would otherwise restore the loser's optimistic value. The inverse is not a re-application of optimism onto fetched truth: it runs only while the cell still stands on the pre-fetch value, and a fetched value replaces the cell wholesale with no inverse outstanding.
