...
use eidola_app_core::{
    ...
    PostReference, PriceInfo, RequestDetail, RequestInfo, SpaceInfo, SpaceMessage, SpendTrailEntry,
};
...
fn register_chat(s: &mut Snapshots) {
    // The "kitchen sink" — the non-interactive visual reference: a realistic
    // multi-party space (two humans + an AI) debating Thrasymachus, exercising
    // every render element — byline gutter with multiple participants, threaded
    // branch + rail, inline reference quote chips, a collaboratively edited
    // (v2) node, corrections both ways, long bodies. Tall window so most of the
    // thread is captured in one frame. See `kitchen_sink_posts`.
    s.add(
        "chat_kitchen_sink",
        size(px(1000.), px(2600.)),
        |window, cx| {
            let core = stub_stores_with_config(cx);
            cx.new(|cx| {
                let mut view = ChatView::new(core, None, WindowInput::new(cx), window, cx);
                view.set_post_tree_for_test(kitchen_sink_posts(), cx);
                view
            })
        },
    );

    // The kitchen sink scrolled to its tail, so the back half is captured in
    // one frame: Mara's draft, the collaboratively edited (v2) newsletter
    // excerpt with its `❝…❞ — re: kimi-k2` reference, the AI's correction on the
    // product, and the indented branch (the tangent off the second post).
    s.add_with_step(
        "chat_kitchen_sink_tail",
        size(px(1000.), px(1100.)),
        |window, cx| {
            let core = stub_stores_with_config(cx);
            cx.new(|cx| {
                let mut view = ChatView::new(core, None, WindowInput::new(cx), window, cx);
        ...
        |cx, window, view| {
            // Put Mara's "draft the excerpt" post (item 4) near the top.
            cx.update_window(window, |_, _, cx| {
                view.update(cx, |v, cx| v.scroll_transcript_for_test(4, 0.0, cx));
            })
            .ok();
        },
    );
...
fn char_range(content: &str, phrase: &str) -> (i64, i64) {
    let byte = content
        .find(phrase)
...
fn kitchen_sink_posts() -> Vec<PostNode> {
