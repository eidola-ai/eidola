...
- **`src/bridge.rs`**: the runtime-agnostic `oneshot`/`mpsc` glue. `bridge(core, f)` runs `f` on the tokio runtime and awaits the result on gpui (used inside store `cx.spawn` tasks). The free functions `chat_stream` / `get_space_messages` / `list_attestations` / … re-home the tokio→oneshot helpers still used by `ChatView`'s streaming and the Record reader — those become entities in step 3; for now the views own the resulting gpui `Task` directly and pull the `Arc<AppCore>` out of their `Stores` bundle.
...
|---|---|---|---|
| `chat.rs` | `ChatView` | `ConfigStore`, `ModelsStore`, `AccountStore`, `WalletStore` (+ `bridge::chat_stream`/`get_space_messages` for streaming/transcript) | Main chat window — message list + input + Send action, plus the onboarding empty states (see [Onboarding](#onboarding--the-chat-windows-empty-states)) |
...
**Chat windows are non-singleton.** Every `NewSpace` invocation opens a fresh `ChatView` via `open_main_window` → `open_chat_window(cx, stores, None)`; with `space_id: None` the view is a blank page whose space is created lazily by the first exchange (protecting the instant-⌘N pillar). The Library reopens an existing space via `open_space_window(cx, stores, space_id)` (same chat window chrome, `Some(id)`): `ChatView::new` stores the id and asynchronously loads the space's persisted messages through the `bridge::get_space_messages` helper on construction, and the next submit continues that space. Opening the same space twice yields two independent windows — no dedup yet, acceptable v1. `open_chat_window` calls `cx.activate(true)` after `cx.open_window` so a window opened from another app's context (dock right-click while a different app is foreground) brings Eidola to the front rather than opening behind.
