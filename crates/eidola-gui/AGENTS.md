...
| `library.rs` | `LibraryView` | `SpacesStore` (index + optimistic archive) | Library window — table of contents of past spaces; reopen / archive |
...
- `blank()` mints an id-less space for ⌘N (instant, no load). The registry **adopts** it once its first exchange assigns an id: `SpacesStore` subscribes to the blank's `StreamEnded`, reads the now-present `id()`, and keys the entity under it — so a later `open(that_id)` joins the same entity.
...
Tail-scroll is **window-local** (each window has its own scroll position) and keys off the space's semantic stream event: `ChatView::on_space_event` matches `SpaceEvent::StreamDelta` (vs the catch-all re-render of `MessagesChanged`/`StreamEnded`) as the hook where a tail re-pin belongs. Today that handler only re-renders (the scroll surface is rebuilt each frame and there is no persistent `ScrollHandle` yet); a future tail policy hangs off this `StreamDelta` arm so it fires per streamed chunk, not per unrelated re-render. Keeping the *semantic* event distinct from the plain `observe` is the point — the doctrine's "tail-scroll only on `StreamDelta`".
...
- **Windowed fetch.** `Listing<T>` per section; each page asks app-core for `PAGE+1 = 51` rows (`LIMIT/OFFSET`) to learn `has_more` without a COUNT. "Load more…" appends. Data flows through the `bridge` free functions (`bridge::list_attestations` / `list_requests` / `spend_trail` / `attestation_detail` / `request_detail`, taking the `Arc<AppCore>` from `Stores::app_core()`) so the view owns its loading state in its own task slots. These window-scoped reads become a reader entity in step 3. Stub stores skip the fetch; tests install rows via `set_*_for_test`.
