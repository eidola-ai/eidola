...
**A color set through a builder method is an override and survives the refresh** (the space view's quoted-passage wash is the live example); everything else follows the theme. A *direct write to a public color field* is not recorded and is re-derived — set colors through the builder. The typography knobs (font size, families, heading callback, paragraph gap, `list_item_gap_factor`, `inline_code_font_family`) are plain fields the refresh never touches.
