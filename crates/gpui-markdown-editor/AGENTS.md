...
- **Host-driven scroll-into-view — `caret_content_y()` / `content_y_for_offset(offset)`.** The editor has **no internal vertical scroll** — it lays out to full `content_height()` and the host scrolls it (the only internal scroll is per-code-block *horizontal*). Both return `Option<(top, bottom)>`: the vertical span relative to the laid-out content top (content-local, unlike the window-absolute IME `bounds_for_range` they derive from). `caret_content_y` answers for the caret, with its wrap-boundary affinity; `content_y_for_offset` answers for **any** source offset — a host revealing something the caret is not at (a search match, a cited passage) — and resolves a boundary **downstream**, since an arbitrary offset carries no affinity. Line ranges are end-inclusive, so an offset can be claimed by two laid-out lines (a code block's rows, a hard-broken paragraph, a table's row boundary): downstream takes the line that *starts* there — the row the reader would say the offset is on — while the caret path keeps the earlier line, which is where an upstream caret renders. Paint-derived like `content_height()`; independent of the runway. An offset past every laid-out range (document end after a trailing newline) falls back to the last line; `None` means no layout yet, so a host over virtualized content must estimate first and correct once it answers.
- **IME composition — `is_composing()`.** True while `marked_range` holds uncommitted preedit text. This editor emits `Change` on **every** preedit keystroke (the marked text really is in the buffer, and a host sizing to content must follow it) — unlike `gpui_component::InputState`, whose preedit path only notifies. So a host that acts on the buffer's *meaning* (searching it, parsing it, sending it) needs one rule, and this is the whole rule: **act on `Change` unless `is_composing()`, because the editor emits at least one final `Change` when a composition ends, however it ends.** Ending it is a commit (`replace_text_in_range`), a caret move that leaves the preedit behind, or the platform dropping the marking without replacement text (`unmark_text` — a click elsewhere while composing, an input-source switch; the marked bytes stay in the buffer as ordinary text). Internally that holds because `marked_range` is cleared in exactly **one** place, `end_composition`, which always emits — so "the marking went away and nobody was told" is not a representable state. And an open composition cannot outlive the editor's ability to *receive* IME, which is the other half of the promise: it ends when the host renders the editor `disabled` (no input handler is registered while read-only — `sync_disabled`) and on the frame focus leaves (gpui routes IME to the focused handle, and Wayland's `text_input` `Leave` tells the client nothing at all — `sync_focused`). Both are transitions, so an editor that was never focused (a headless test driving the IME methods directly) is left alone. *At least* one rather than exactly one is the honest promise: an ending that also changes the buffer reports the ending and then the edit. The stronger wording cannot be kept, because suppressing the second would mean promising, at the moment the marking is cleared, something only knowable afterwards — whether the edit produced a buffer delta at all. A commit of the same bytes (accepting romaji or ASCII unchanged) produces none. Acting on `Change` must therefore be idempotent, which it must be anyway: preedit keystrokes emit `Change` too. **The boundary is the frame.** The editor notices those transitions only when it renders, so two lifecycle exits are the host's: an editor **dropped** mid-composition (nothing left to ask, and no subscriber left to tell), and one the host **stops rendering** while keeping its state — a virtualized list dropping a row. For the second, call `MarkdownEditorState::end_composition`, which is public for exactly that. The `EntityInputHandler::marked_text_range` half of the IME contract needs a `Window` and is not a host-facing query.
...
| `style.rs` | `MarkdownStyle` + the `theme_colors!` registry — every theme-derived color, declared once |
...
The editor carries **no color palette of its own**. Every theme-sourced color is declared once in `style.rs`'s `theme_colors!` registry — name, field, derivation — and that one list is what `MarkdownStyle::from_theme` builds from, what `refresh_theme_colors` re-derives, and what records an override. The element's `render` calls `refresh_theme_colors` each frame, so a style a host built once still follows a live mode flip and **no color can be derived in one place and forgotten in the other** (the hand-maintained refresh list this replaced had drifted both ways: the highlight washes and the table rule were never refreshed, and the four colors a caller can set were re-derived unconditionally, silently discarding the override).

**A color set through a builder method is an override and survives the refresh** (the space view's quoted-passage wash is the live example); everything else follows the theme. Every registered color has a builder, because the registry *generates* them — registering a color and leaving it un-overridable is not a state the crate can be in. A *direct write to a public color field* is not recorded and is re-derived, so set colors through the builder. The typography knobs (font size, families, heading callback, paragraph gap, `list_item_gap_factor`, `inline_code_font_family`) are plain fields the refresh never touches.
