...
- `www/blog/YYYY-MM-DD-<slug>.md` — blog posts at `/blog/<slug>/`, plus a generated index at `/blog/` and an Atom feed at `/blog/atom.xml`. Front matter is TOML between `+++` fences (`title`, `description`, `date`, `draft`); see the committed draft example post for the format. If no posts are published, the index, feed, and Blog nav/feed links are not generated at all.
