...
//! | `run_turn`/`run_turn_stream` — the ceiling bound a round that **had already written answer text** | Exactly what an ordinary turn writes: the `inference`, its blocks, the request row | The ordinary success set — `Space(id)`, `Record`, `Wallet` where the turn spent. No new exit point: the partial answer is real text and is kept, and the only difference is that `ChatResult::truncated` says it is not finished, so no surface has to guess | `chat_path.rs` (`a_partial_answer_at_the_ceiling_is_kept_and_reported_as_truncated`, `an_ordinary_turn_is_not_reported_as_truncated`) |
