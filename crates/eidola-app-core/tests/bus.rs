...
//! | Funding failure at request time (`NoAccount`, `InsufficientBalance`, zero-charge) | The posted user turn (post committed it) | `Space(id)`, `SpaceIndex` (from post); error wraps the space id | `chat_path.rs` (`no_account_persists_post_*`, `insufficient_balance_persists_post_*`, `a_foreign_issued_credential_is_never_spent_against_another_server`) — root() still routes onboarding; the post survives. Credits issued by a *different* server reach this same exit (selection is issuer-scoped), deliberately not an error class of their own |
