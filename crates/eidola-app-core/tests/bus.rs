...
//! | The **sub-space turn driver** — the owner's **report** delivered into the parent (the delegation's one terminal act, whichever of the four outcomes ended it) | An `inference` by the owning agent in the parent, replying to its own last post there, carrying a `relation='reference'` edge at ordinal 1 that quotes the delegated room's last word — attached by the driver at the authoring seam and annotated with what ended the room | `Space(parent_id)`, `Record`, `Wallet` where the parent's backend spends — an ordinary turn's emissions, again all `Unattended`. **No `SpaceIndex`** (`post` owns that, and the driver posts nothing) | `subspace_driver.rs` (`a_finished_delegation_reports_back_with_the_rooms_last_word_attached`, `a_room_that_pauses_at_its_cascade_guard_says_so`, `a_delegation_stops_at_its_turn_budget_and_says_so`, `a_failed_turn_is_reported_rather_than_swallowed`) |
//! | The **sub-space turn driver** — nothing to do: the room is archived, is not a sub-space at all, or its last word has already been reported (the derived "is there work outstanding" read) | Nothing at all — the check is three reads and precedes every planning call, so an armed driver on a settled room starts no engine, sends no request and writes no row | **No emissions** | `subspace_driver.rs` (`the_driver_never_drives_an_archived_room`, `a_spent_budget_survives_a_restart`) |
//! | The **sub-space turn driver** — the **parent** is archived, so the report cannot be delivered | The room's own driven turns (which are unaffected — archiving a parent does not archive its delegations); the report itself writes nothing, refused by `prepare_turn`'s archival gate before any spend | The driven turns' `Space(sub_id)` + `Record`; nothing for the parent. The room's last word stays unreported, which is what a later run reads as still outstanding | `subspace_driver.rs` (`a_report_into_an_archived_parent_is_refused_and_stays_outstanding`) |
//!
