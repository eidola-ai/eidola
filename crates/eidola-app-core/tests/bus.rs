...
//! | `run_turn`/`run_turn_stream` — **structurally unusable `tool_calls`** (no call id / no function name) | The posted user turn; the round's raw request row, attached to **no action** (nothing could be written as a `tool_use` block) | `Space(id)`, `Record`; **error is `AppError::ToolLoop`**, wrapped with the space id | `chat_path.rs` (`structurally_malformed_tool_call_fails_the_turn_honestly`) |
//!
