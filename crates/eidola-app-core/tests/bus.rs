...
//! | The **sub-space turn driver** — the owner's **report** delivered into the parent (the delegation's one terminal act, whichever of the four outcomes ended it) | An `inference` by the owning agent in the parent, replying to its own last post there, carrying a `relation='reference'` edge at ordinal 1 that quotes **the post the walk ended on** (its witness, never a fresh read of the tail) — attached by the driver at the authoring seam, annotated with a *token* naming what ended the room (`DelegationEnd::token`, never a sentence), and attached beneath the owner's own answer to the post the delegation was opened from. A **regeneration** of that report replicates the edge onto the new generation at its own ordinal (`prepare_turn` on `Revise`, the rule `edit_post` already follows), so re-wording a report neither loses its finding nor re-opens the delegation. `BudgetSpent` can bind on the walk's planning hops, not only on persisted `inference`/`decision` rows: an empty router plan writes nothing, so the durable meter would not stop a burst of watched-room posts from billing an unbounded number of router calls | `Space(parent_id)`, `Record`, `Wallet` where the parent's backend spends — an ordinary turn's emissions, again all `Unattended`. **No `SpaceIndex`** (`post` owns that, and the driver posts nothing) | `subspace_driver.rs` (`a_finished_delegation_reports_back_with_the_rooms_last_word_attached`, `a_room_that_pauses_at_its_cascade_guard_says_so`, `a_delegation_stops_at_its_turn_budget_and_says_so`, `empty_router_hops_still_spend_the_delegation_budget`, `a_failed_turn_is_reported_rather_than_swallowed`) |
