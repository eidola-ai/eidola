...
//! | `prepare_turn` — the selection names a model the backend's catalog **no longer lists** (a row retired upstream; nothing un-picks the participant, the space router, or the model a post recorded and a regeneration replays) | The posted user turn where a combined ask committed one; **nothing at all** for `respond_stream_as` and `regenerate`, which post nothing. The refusal lands after the catalog fetch and before the credential, so no hold, no request row, no generation — and the answer a regeneration would have replaced is intact | `Space(id)`, `SpaceIndex` (from post) and nothing else. **Error is `AppError::ModelUnavailable`**, carrying the selection as it was persisted. **No substitution is possible from here** — the turn never reaches a completion request, which is what the tests assert | `chat_path.rs` (`a_turn_whose_model_left_the_catalog_is_refused_by_name`, `a_streaming_turn_whose_model_left_the_catalog_is_refused_by_name`, `a_regeneration_replaying_a_retired_catalog_row_is_refused_by_name` — each pins zero completion requests); `participants_orchestration.rs` (`a_router_whose_model_left_the_catalog_degrades_without_substituting`, the chore half: the router swallows it and lets the post through unrefined rather than routing through another model) |
...
//! | `run_turn_stream` — the SSE body **ended without ever saying it was over** (no `[DONE]` and no terminal `finish_reason` on any chunk; every byte well-formed, so nothing failed in the transport — unlike the mid-frame abort, which dies in the decoder) | The posted user turn; the round's raw request row, attached to **no action**. Deliberately **no `inference`**: the upstream never claimed a completion, and the durable marker that could have said otherwise (`action.truncated`) records the upstream's own *ceiling* verdict, which a dropped connection never issued — under `Revise` it would also have superseded an intact answer with severed text | `Space(id)`, `Record`; `Wallet` at spend start, and the hold settles as it does for any turn that ran. **Error is `AppError::Network`** — a transport failure with partial data, not a truncation (no ceiling was reached) | `chat_path.rs` (`a_stream_that_never_says_it_finished_writes_no_answer`; `a_regeneration_whose_stream_never_finished_keeps_the_answer_it_would_replace` for the generation the rule protects) |
//! | `run_turn`/`run_turn_stream` — the round reached the **completion ceiling before writing any answer** (`finish_reason: "length"`, no content, no tool calls — the reasoning model that thinks until the budget is gone) | The posted user turn; the round's raw request row, attached to **no action**. Deliberately **no `inference`**: a generation with nothing readable in it would be current, would take a reply, and — under `Revise` — would have superseded a real answer with an empty one | `Space(id)`, `Record`; `Wallet` at spend start, and the hold settles exactly as a turn that produced text settles it (the tokens went). **Error is `AppError::ResponseTruncated`**, carrying the ceiling reached | `chat_path.rs` (`a_ceiling_exit_with_no_answer_writes_no_generation_blocking`, `..._streaming` for the transport agreement, `a_regeneration_cut_off_at_the_ceiling_keeps_the_answer_it_would_replace` for the generation the rule protects) |
//! | `run_turn`/`run_turn_stream` — the ceiling bound a round that **had already written answer text** | Exactly what an ordinary turn writes: the `inference` — **with `truncated = 1` on its row** — its blocks, the request row | The ordinary success set — `Space(id)`, `Record`, `Wallet` where the turn spent. No new exit point: the partial answer is real text and is kept, and the only difference is that the turn says it is not finished. It says so **twice from one classification**, made after tool assembly where the ceiling, the content and the absence of tool calls are all known: durably on the answer's row (so a reader who reopens the space still sees the mark) and on `ChatResult::truncated` (so the window watching the live turn need not wait for the reload). Neither is derived from the other, so they cannot drift | `chat_path.rs` (`a_partial_answer_at_the_ceiling_is_kept_and_reported_as_truncated`, `a_cut_off_answer_still_says_so_after_a_restart`, `an_ordinary_turn_is_not_reported_as_truncated`) |
//! | `regenerate_stream` — a **second regeneration of the same post** while one runs in this process | Nothing at all. The claim is taken after the (free) target/kind/membership gates and **before the credential**, so a refusal costs no request and no spend — unlike the durable single-successor index, which rejects the loser only after both have paid | **No emissions**; error is `AppError::RegenerationInFlight` | `chat_path.rs` (`a_second_regeneration_of_one_post_is_refused_before_it_spends`; `a_finished_regeneration_releases_its_claim` for the release) |
...
//!
//! | `spawn_subspace` — an **agent-spawned sub-space minted** (the delegation door; see `subspaces`). One transaction writes the whole room: the `space` row (parent link, the parent's cascade limit and router, born stamped), the owner's `role = 'owner'` membership, each sub-agent's membership with `override_notify_policy = 'all'`, the **brief** as the first post (an agent-authored `brief` action, which is why the space is never pristine), and the capability snapshot | `SpaceIndex` (the Library lists sub-spaces like any other conversation), `Space(sub_id)` (it has a transcript to read) and `Participants` (a roster was written) — one per thing the instantiation wrote, the `create_space` rule. **Nothing about the parent**, which gained nothing. A refusal — a guard, an attenuation check, an ineligible or model-less participant, an anchor the parent does not currently show — rolls back to nothing and emits nothing at all. Every door that acts as the human **on work already in such a room** — `edit_post`, `regenerate`, `respond_stream` — is refused before any write *and before any spend* (`AppError::NotJoined`), so each commits nothing and emits nothing | `subspaces.rs` (`a_spawn_opens_a_titled_room_with_no_human_in_it`; `a_refused_spawn_writes_nothing_at_all` for the zero-trace arm; `a_spawn_cannot_anchor_to_a_superseded_wording` and `subspace_driver.rs`'s `a_spawn_cannot_anchor_to_a_hidden_tip` for the current-post predicate) |
//! | `post` — the shared human's **first word in a delegated room**, which joins them to it. The membership rides the post's own transaction (`db::PostPlan::join_author`, insert-or-revive on the space's primary key, so a live one is left exactly as it stands) rather than a step in front of it: the roster the models are shown has to be true of the transcript they read, and any gap between the two writes is a window in which a turn can be planned over a post by somebody the roster omits | The post's own writes, plus a `space_participant` row for the human at `role = 'member'` — never `owner`, which is the delegation's structural role | `Space(id)` + `SpaceIndex`? as any post, **plus `Participants` exactly when the membership was written**. A second post into the same room writes no second membership and announces nothing about the roster | `subspaces.rs` (`a_humans_first_post_into_a_subspace_joins_them_to_it`; `the_roster_of_a_delegated_room_names_the_reader_who_joined_it` for the wire bytes the models read) |
//! | `chat`/`chat_stream` — asked in a **live sub-space** (`AppError::DrivenConversation`) | Nothing at all. Combined post-and-turn is a cascade, and those rooms belong to the driver; the gate lands before `post`, so a refusal is zero-trace. The refusal is the same whoever asks: it no longer looks at membership first, because posting is now what joins a reader, so a membership question in front of this one would demand a step for a verb refused either way and would name the wrong obstacle | **No emissions** | `subspaces.rs` (`a_combined_ask_in_a_live_subspace_is_refused`, `a_humans_first_post_into_a_subspace_joins_them_to_it`); `subspace_driver.rs` (`a_combined_ask_does_not_double_drive_a_delegated_room`) |
//! | `delegate` — the **turn-scoped tool** that reaches the spawn door mid-loop (see `subspaces`). Gated `scope == 'global' && backend_accepts_tools`, the same structural gate `list_my_spaces` carries; the owner is the responding participant, the parent is the turn's space, and the anchor is the post the turn is answering | Exactly what `spawn_subspace` writes, in its one transaction — the tool adds no write and no guard of its own | The spawn's three: `SpaceIndex`, `Space(sub_id)`, `Participants`, mid-loop. Every refusal the door decides — a guard, the attenuation check, an ineligible participant, a seat that has left the parent since the turn's roster was snapshotted — comes back as a **tool result** rather than a turn failure, and rolls back to nothing, so a refused call emits nothing at all | `subspaces.rs` (`the_delegate_tool_opens_a_room_from_the_turn_it_was_called_in`, `the_delegate_tool_is_refused_by_every_guard_the_door_holds`, `a_delegation_can_only_name_the_conversations_own_roster`, `a_seat_that_leaves_mid_turn_is_refused_rather_than_seated`, `a_space_owned_agent_is_offered_no_delegation`) |
//!
...
//!
//! | **A proxied completion** (`proxy::route`) — the local inference proxy's space-free turn, blocking or streaming. It is a *sibling* of the chore runner and not a rider on the turn machinery: no `prepare_turn`, no `TurnPrep`, no context assembly, and therefore **no row in this table's turn section can apply to it**. What it shares with a turn is the paying and the recording | `Record` per exchange — the request row, `action_id` **unset**, `backend_id` set, carrying the credential nonce it spent; plus a second `Record` where an `eidola` route's handshake wrote an `attestation` + `connection` pair. `Wallet` at spend start (`acquire_spend`'s own emission) and again when the hold settles. **Nothing at the semantic layer, ever** — no `Space(id)`, no `SpaceIndex`, no `Participants`: a proxied request creates no conversation and writes no action, which is the whole contract | `proxy.rs` (`a_proxied_completion_pays_records_and_creates_no_conversation` — the request row with no action and no space, the settled credential, and the Library unchanged; `a_streaming_completion_forwards_the_upstreams_own_events` for the SSE arm, whose spend always settles through the recovery endpoint) |
//!
//! | **A proxied completion that never reached the upstream** — an unexposed backend, a stale model reference, a `local` model the exposure setting will not start, a network error, or a non-2xx | For the refusals decided before the route opens (exposure, `ModelUnavailable`): **nothing at all** — no credential, no request, no engine started. For a failure after the request left: the raw exchange as a `Record` request row attached to no action, with `error` set where the transport failed, and the hold settled or recovered exactly as a turn's is | `proxy.rs` (`only_the_backends_the_reader_ticked_are_offered`, `only_loaded_means_a_request_cannot_start_an_engine`, `a_malformed_request_is_refused_and_the_connection_survives_it`) |
//!
//! | **The proxy's own configuration** — settings, the exposed-backend set, a key generated or revoked | `Proxy`, once per durable write, and nothing else. A revoke that found no live row emits nothing (the decide-at-the-write rule), and a refused settings write commits nothing and says nothing | `proxy.rs` (`the_settings_round_trip_and_refuse_what_cannot_be_bound`, `a_key_is_shown_once_and_stored_only_as_a_digest`, `a_revoked_key_stops_working_and_a_live_one_does_not`) |
//!
//! Those are the two places a *successful* tool round emits: the round itself
//! is still not an exit point (see below), but a tool's own durable write is a
//! commit like any other. `remember` is safe mid-loop for the same reason the
//! round's trace rows are — `memory` is not a post type, so no subscriber's
//! rendered thread changes underneath it. `delegate` is louder and is safe for
//! a different reason: everything it announces is about a room that did not
//! exist a moment ago, so no subscriber was reading any of it, and the one
//! surface that *was* — the Library — is announced by exactly the emission
//! that would tell it a new conversation had appeared. Nothing about the space
//! the turn is running in changes.
//!
...
//!
//! **Regeneration has two transports and one contract.** `AppCore::regenerate`
//! and `AppCore::regenerate_stream` share `Inner::begin_regeneration` — the same
//! target/tip resolution, the same two gates (the human's verb, on an agent's
//! inferred answer), the same in-flight claim — and then run the blocking or
//! streaming turn loop with `ResponseMode::Revise`. So the streaming door adds
//! **no durable exit point**: it hits every `run_turn`/`run_turn_stream` row
//! above, with no leading `post` (so no `SpaceIndex`), exactly as
//! `respond_stream` does for `Reply`. Covered by
//! `chat_path.rs` (`streaming_regeneration_revises_in_place_and_streams_its_deltas`).
//!
