...
//! | `delegate` — the **turn-scoped tool** that reaches the spawn door mid-loop (see `subspaces`). Gated `scope == 'global' && backend_accepts_tools`, the same structural gate `list_my_spaces` carries; the owner is the responding participant, the parent is the turn's space, and the anchor is the post the turn is answering | Exactly what `spawn_subspace` writes, in its one transaction — the tool adds no write and no guard of its own | The spawn's three: `SpaceIndex`, `Space(sub_id)`, `Participants`, mid-loop. Every refusal the door decides — a guard, the attenuation check, an ineligible participant — comes back as a **tool result** rather than a turn failure, and rolls back to nothing, so a refused call emits nothing at all | `subspaces.rs` (`the_delegate_tool_opens_a_room_from_the_turn_it_was_called_in`, `the_delegate_tool_is_refused_by_every_guard_the_door_holds`, `a_delegation_can_only_name_the_conversations_own_roster`, `a_space_owned_agent_is_offered_no_delegation`) |
//!
