...
`apply` + `inspect` are also what makes the attestation's `apple_signature_reconstructs` claim mechanical rather than merely affirmable: `release-tool attest` reconstructs the signed bundle from the manifest-recorded unsigned build plus the published detached material, requires it to equal the shipped artifact entry for entry, reads back the identity, and refuses to offer the claim otherwise (`crates/release-tool/src/apple_verify.rs`, exercised against this crate's ad-hoc-signed fixture — the claim text names no Developer ID, so the chain is testable before any certificate exists). Inspection is also where the signing recipe is enforced: a reconstruction whose bundle lacks the hardened runtime is refused rather than recorded, since an installed client asserts that flag instead of reading it. The `settled` fixture is the negative — an ad-hoc bundle signed without `--options runtime`, so detaching it against itself produces valid material whose reconstruction is wrong in exactly that one way.
