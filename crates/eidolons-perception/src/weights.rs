//! Weight loading utilities for Llama and Qwen3 models from safetensors files.
...
use crate::qwen3::{Qwen3, Qwen3Config, attention::Qwen3Attention, model::Qwen3Layer};
...
/// Qwen3 attention layer weights, including QK-Norm weights.
pub struct Qwen3AttentionWeights<B: Backend> {
    pub q_proj: Param<Tensor<B, 2>>,
    pub k_proj: Param<Tensor<B, 2>>,
    pub v_proj: Param<Tensor<B, 2>>,
    pub o_proj: Param<Tensor<B, 2>>,
    /// QK-Norm weight for query
    pub q_norm: Param<Tensor<B, 1>>,
    /// QK-Norm weight for key
    pub k_norm: Param<Tensor<B, 1>>,
}

/// Weight loader for Qwen3 models.
///
/// Supports both single-file and multi-shard safetensors files.
pub struct Qwen3WeightLoader<'a> {
    /// Map of tensor names to their shard data
    shards: Vec<SafeTensors<'a>>,
    /// Index mapping tensor names to shard indices
    tensor_index: HashMap<String, usize>,
}

impl<'a> Qwen3WeightLoader<'a> {
    /// Creates a new weight loader from a single safetensors file.
    pub fn new(data: &'a [u8]) -> Result<Self> {
        let tensors =
            SafeTensors::deserialize(data).context("Failed to deserialize safetensors file")?;
        ...
        for name in tensors.names() {
            tensor_index.insert(name.clone(), 0);
        }
    ...
    /// Creates a new weight loader from multiple safetensors shard files.
    ///
    /// # Arguments
    ///
    /// * `shard_data` - Slice of tuples containing (shard_filename, shard_bytes)
    pub fn from_shards(shard_data: &'a [(&str, &'a [u8])]) -> Result<Self> {
        let mut shards = Vec::with_capacity(shard_data.len());
        let mut tensor_index = HashMap::new();

        for (shard_idx, (filename, data)) in shard_data.iter().enumerate() {
            let tensors = SafeTensors::deserialize(data)
                .with_context(|| format!("Failed to deserialize shard: {}", filename))?;

            for name in tensors.names() {
                tensor_index.insert(name.clone(), shard_idx);
            }
    ...
    /// Lists all tensor names across all shards.
    pub fn tensor_names(&self) -> Vec<String> {
        self.tensor_index.keys().cloned().collect()
    }

    /// Loads a tensor by name, looking up which shard contains it.
    fn load_tensor_by_name<B: Backend, const D: usize>(
        &self,
        name: &str,
        device: &B::Device,
    ) -> Result<Tensor<B, D>> {
        let shard_idx = self
            .tensor_index
            .get(name)
            .with_context(|| format!("Tensor '{}' not found in any shard", name))?;

        load_tensor(&self.shards[*shard_idx], name, device)
    }

    /// Loads a 1D tensor as a Param.
    fn load_param_1d_by_name<B: Backend>(
        &self,
        name: &str,
        device: &B::Device,
    ) -> Result<Param<Tensor<B, 1>>> {
        let tensor = self.load_tensor_by_name::<B, 1>(name, device)?;
        Ok(Param::from_tensor(tensor))
    }

    /// Loads a 2D tensor as a Param, transposing from HuggingFace format to Burn format.
    fn load_linear_weight_by_name<B: Backend>(
        &self,
        name: &str,
        device: &B::Device,
    ) -> Result<Param<Tensor<B, 2>>> {
        let tensor = self.load_tensor_by_name::<B, 2>(name, device)?;
        let transposed = tensor.transpose();
        Ok(Param::from_tensor(transposed))
    }
    ...
    /// Loads the token embedding weights.
    pub fn load_embed_tokens<B: Backend>(&self, device: &B::Device) -> Result<Param<Tensor<B, 2>>> {
        let tensor = self.load_tensor_by_name::<B, 2>("model.embed_tokens.weight", device)?;
        Ok(Param::from_tensor(tensor))
    }

    /// Loads the final layer norm weights.
    pub fn load_final_norm<B: Backend>(&self, device: &B::Device) -> Result<Param<Tensor<B, 1>>> {
        self.load_param_1d_by_name("model.norm.weight", device)
    }

    /// Loads the LM head weights.
    /// Note: Qwen3 always has separate lm_head (not tied to embeddings).
    pub fn load_lm_head<B: Backend>(&self, device: &B::Device) -> Result<Param<Tensor<B, 2>>> {
        self.load_linear_weight_by_name("lm_head.weight", device)
    }

    /// Loads attention weights for a specific layer, including QK-Norm weights.
    pub fn load_attention_weights<B: Backend>(
        &self,
        layer_idx: usize,
        device: &B::Device,
    ) -> Result<Qwen3AttentionWeights<B>> {
        let prefix = format!("model.layers.{}.self_attn", layer_idx);

        Ok(Qwen3AttentionWeights {
            q_proj: self
                .load_linear_weight_by_name(&format!("{}.q_proj.weight", prefix), device)?,
            k_proj: self
                .load_linear_weight_by_name(&format!("{}.k_proj.weight", prefix), device)?,
            v_proj: self
                .load_linear_weight_by_name(&format!("{}.v_proj.weight", prefix), device)?,
            o_proj: self
                .load_linear_weight_by_name(&format!("{}.o_proj.weight", prefix), device)?,
            // QK-Norm weights
            q_norm: self.load_param_1d_by_name(&format!("{}.q_norm.weight", prefix), device)?,
            k_norm: self.load_param_1d_by_name(&format!("{}.k_norm.weight", prefix), device)?,
        })
    }
    ...
    pub fn load_mlp_weights<B: Backend>(
        ...
        Ok(MlpWeights {
            gate_proj: self
                .load_linear_weight_by_name(&format!("{}.gate_proj.weight", prefix), device)?,
            up_proj: self
                .load_linear_weight_by_name(&format!("{}.up_proj.weight", prefix), device)?,
            down_proj: self
                .load_linear_weight_by_name(&format!("{}.down_proj.weight", prefix), device)?,
        })
    }
    ...
    pub fn load_layer_norms<B: Backend>(
        ...
        Ok(LayerNormWeights {
            input_layernorm: self
                .load_param_1d_by_name(&format!("{}.input_layernorm.weight", prefix), device)?,
            post_attention_layernorm: self.load_param_1d_by_name(
                &format!("{}.post_attention_layernorm.weight", prefix),
                device,
            )?,
        })
...
/// Loads a complete Qwen3 model from safetensors data.
///
/// # Arguments
///
/// * `data` - Raw bytes of the safetensors file
...
/// # Returns
///
/// A Qwen3 model with weights loaded from the safetensors file.
pub fn load_qwen3_from_safetensors<B: Backend>(
    data: &[u8],
    config: &Qwen3Config,
    device: &B::Device,
) -> Result<Qwen3<B>> {
    let loader = Qwen3WeightLoader::new(data)?;
    load_qwen3_from_loader(&loader, config, device)
}

/// Loads a complete Qwen3 model from sharded safetensors files.
///
/// # Arguments
///
/// * `shard_data` - Slice of tuples containing (shard_filename, shard_bytes)
...
/// # Returns
///
/// A Qwen3 model with weights loaded from the safetensors shards.
pub fn load_qwen3_from_shards<B: Backend>(
    shard_data: &[(&str, &[u8])],
    config: &Qwen3Config,
    device: &B::Device,
) -> Result<Qwen3<B>> {
    let loader = Qwen3WeightLoader::from_shards(shard_data)?;
    load_qwen3_from_loader(&loader, config, device)
}

/// Internal function to load Qwen3 model from a weight loader.
fn load_qwen3_from_loader<B: Backend>(
    loader: &Qwen3WeightLoader<'_>,
    config: &Qwen3Config,
    device: &B::Device,
) -> Result<Qwen3<B>> {
    // Load embeddings
    ...
    let mut layers = Vec::with_capacity(config.num_hidden_layers);
    for layer_idx in 0..config.num_hidden_layers {
        let layer = load_qwen3_layer(loader, layer_idx, config, device)?;
        layers.push(layer);
    }
    ...
    // Load LM head (always separate in Qwen3)
    let lm_head_weight = loader.load_lm_head(device)?;

    Ok(Qwen3::from_weights(
        embed_tokens,
        layers,
        norm,
        lm_head_weight,
...
/// Loads a single Qwen3 transformer layer from the weight loader.
fn load_qwen3_layer<B: Backend>(
    loader: &Qwen3WeightLoader<'_>,
    layer_idx: usize,
    config: &Qwen3Config,
    device: &B::Device,
) -> Result<Qwen3Layer<B>> {
    // Load attention weights (including QK-Norm)
    let attn_weights = loader.load_attention_weights(layer_idx, device)?;
    let self_attn = Qwen3Attention::from_weights(
        attn_weights.q_proj,
        attn_weights.k_proj,
        attn_weights.v_proj,
        attn_weights.o_proj,
    ...
    // Load MLP weights (same as Llama)
    let mlp_weights = loader.load_mlp_weights(layer_idx, device)?;
    let mlp = LlamaMlp::from_weights(
        mlp_weights.gate_proj,
        mlp_weights.up_proj,
