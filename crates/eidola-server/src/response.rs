//! Eidola-specific response types with inline privacy and verification metadata.
//!
//! These types define the public API contract between the eidola server and its clients.
...
/// An Eidola chat completion response.
#[derive(Debug, Clone, Serialize, ToSchema)]
pub struct EidolaResponse {
...
impl EidolaResponse {
    ...
    pub fn from_completion(
        ...
        Self {
            id: response.id,
            object: "eidola.chat.completion".to_string(),
...
pub struct DataExposure {
    /// The party (e.g., "eidola-proxy", "phala-gpu-tee", "anthropic").
    pub party: String,
...
///
/// Clients identify this by checking `object == "eidola.chat.completion.metadata"`.
/// Standard OpenAI clients will ignore it.
#[derive(Debug, Clone, Serialize, ToSchema)]
pub struct EidolaStreamMetadata {
    /// The object type.
    pub object: String,
...
impl EidolaStreamMetadata {
    pub fn new(
        ...
        Self {
            object: "eidola.chat.completion.metadata".to_string(),
            id,
            privacy,
            verification,
            refund,
...
mod tests {
    ...
    #[test]
    fn test_eidola_response_serialization() {
        let response = EidolaResponse {
        ...
        assert!(json.contains("\"object\":\"eidola.chat.completion\""));
    ...
    fn test_stream_metadata_serialization() {
        let meta = EidolaStreamMetadata::new(
            "eid-123".to_string(),
            PrivacyMetadata {
                authorization: AuthorizationInfo {
        ...
        assert!(json.contains("\"object\":\"eidola.chat.completion.metadata\""));
    }
