...
- **Client-directed tracing lives in one place**, `requested_trace_parent` in `middleware.rs`: a hand-parsed `traceparent` whose sampled flag is honored, and nothing else. **Do not install a `TextMapPropagator`** — a global one also injects trace context into the server's outbound calls, putting the trace id on requests to Tinfoil, Stripe, and GitHub. Today no outbound request carries trace context. The sampler must stay unable to promote a trace on its own; it only propagates middleware's decision to child spans.
