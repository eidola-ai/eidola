...
' "$APP/Contents/MacOS/Eidola"
                ...
                # The app icon, named to match CFBundleIconFile. Referenced as
                # a direct path for the same reason as Info.plist: the icon's
                # bytes then participate in the derivation hash.
          ...
          if eidolaGuiLinux == null then
            ...
            pkgs.runCommand "eidola-gui-linux"
              ...
              ''
                ...
                # SVG every modern shell prefers plus fixed-size PNGs for the
                # ones that don't rasterize. Generated by `just update-brand`.
