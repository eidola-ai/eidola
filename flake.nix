          ...
          if !isDarwin || swiftPkg == null then
            ...
            pkgs.stdenv.mkDerivation {
              ...
              installPhase = ''
                ...
                # Copy committed .icns if present (generated by: just update-app-icon)
                if [ -f apps/macos/Support/AppIcon.icns ]; then
                  cp apps/macos/Support/AppIcon.icns "$APP/Contents/Resources/"
                fi
