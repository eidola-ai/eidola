...
if [[ -z "$SOURCE_XCFRAMEWORK" ]]; then
  ...
  (cd "$REPO_ROOT" && cargo build -p eidola-app-core --release --target aarch64-apple-darwin)

  echo "Building for x86_64-apple-darwin..."
  (cd "$REPO_ROOT" && cargo build -p eidola-app-core --release --target x86_64-apple-darwin)
  ...
  LIB_ARM64="$TARGET_DIR/aarch64-apple-darwin/release/libeidola_app_core.a"
  LIB_X86_64="$TARGET_DIR/x86_64-apple-darwin/release/libeidola_app_core.a"
...
cp -R "$SOURCE_XCFRAMEWORK/libeidola_app_core-rs.xcframework" "$DEST"
chmod -R +w "$DEST"
